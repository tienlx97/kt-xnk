...
| 2026-08-15 | Defer task 4.1 (`Sandpack`/`SandpackRSC`/`SandpackWithHTMLOutput`, requires `@codesandbox/sandpack-react`); ship task 5.1 (`LanguageList`/`TeamMember`/`ErrorDecoder`) with minimal placeholder data instead of real translation/team/error-code data. | User decision: no KT-XNK content uses an interactive sandbox yet, so the new-dependency/bundle-measurement decision is postponed until one does. KT-XNK has no equivalent translation program, team roster, or error-code database, so placeholder data satisfies the "compatible props" adaptation without inventing false company data. |
